...
export async function onRequest(context){const {request,env}=context;try{const url=new URL(request.url),mode=String(url.searchParams.get("mode")||"");
  if(mode==="healthz"&&request.method==="GET"){
    ...
    return json({ok:true,db:true,kv:!!env.KV,revision,apiVersion:"18.7"});
  }
  ...
  if(mode==="bootstrap"&&request.method==="GET"){
    ...
    if(session){const source=await migrateFromV16(env,owner);if(seeded&&!await getSetting(env,owner,"revision"))await setSetting(env,owner,"revision","1");return json(await buildBootstrap(env,owner,source,{publicView:false}))}
...
}catch(e){return json({ok:false,error:"V18.7 API error: "+String(e?.message||e)},500)}}
